using FluentValidation.TestHelper;

using JetBrains.Annotations;

using MediaService.Controllers.Requests;
using MediaService.Controllers.Validators;

namespace MediaService.Tests.Controllers.Validators;

[TestSubject(typeof(GetFilesRequestValidator))]
public class GetFilesRequestValidatorTest
{
    private readonly GetFilesRequestValidator _validator = new();

    [Fact]
    public void Validate_WithValidRequest_ReturnsNoValidationError()
    {
        // Arrange
        var request = new GetFilesRequest("my-bucket", "folder1/");

        // Act
        var result = _validator.TestValidate(request);

        // Assert
        result.ShouldNotHaveValidationErrorFor(x => x.Prefix);
    }
}
